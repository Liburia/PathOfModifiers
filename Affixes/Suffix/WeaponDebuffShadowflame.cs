...
{
    public class WeaponDebuffShadowflame : Suffix, ITieredStatFloatAffix
    {
        ...
        {
            TooltipLine line = new TooltipLine(mod, "WeaponDebuffShadowflame", $"[T{tierText}] {(int)Math.Round(multiplier * 100)}% chance to shadowflame enemies for 3-10 seconds on hit");
        ...
        public override void OnHitNPC(Item item, Player player, NPC target, int damage, float knockBack, bool crit)
        {
            if (player.HeldItem == item)
            {
                ...
                {
                    target.AddBuff(153, Main.rand.Next(90, 330));
                }
            }
        }
