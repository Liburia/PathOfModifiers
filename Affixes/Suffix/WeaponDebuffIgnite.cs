...
{
    public class WeaponDebuffIgnite : Suffix, ITieredStatFloatAffix
    {
        ...
        {
            TooltipLine line = new TooltipLine(mod, "WeaponDebuffIgnite", $"[T{tierText}] {(int)Math.Round(multiplier * 100)}% chance to ignite enemies for 3-10 seconds on hit");
        ...
        public override void OnHitNPC(Item item, Player player, NPC target, int damage, float knockBack, bool crit)
        {
            if (player.HeldItem == item)
            {
                ...
                {
                    target.AddBuff(24, Main.rand.Next(90, 330));
                }
            }
        }
