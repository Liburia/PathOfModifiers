...
{
    public class WeaponDebuffIchor : Suffix, ITieredStatFloatAffix
    {
        ...
        {
            return $"[T{tierText}] {(int)Math.Round(multiplier * 100)}% chance to ichor enemies for 3-10 seconds on hit";
        }
        ...
        public override void OnHitNPC(Item item, Player player, NPC target, int damage, float knockBack, bool crit)
        {
            if (player.HeldItem == item)
            {
                ...
                {
                    target.AddBuff(69, Main.rand.Next(90, 330), false);
                }
            }
        }
