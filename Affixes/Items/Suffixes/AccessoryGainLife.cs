...
namespace PathOfModifiers.Affixes.Items.Suffixes
{
    public class AccessoryGainLife : AffixTiered<TTInt, TTFloat>, ISuffix
    {
        ...
        public override string GetAffixText(bool useChatTags = false)
        {
            ...
            return Language.GetText("Mods.PathOfModifiers.Affixes.Suffixes.AccessoryGainLife").Format( gainLose ,  valueRange1 ,  valueRange2 );
        }
        ...
        void Heal(Item item, Player player)
        {
            if (ItemItem.IsAccessoryEquipped(item, player) && (Main.GameUpdateCount - lastProcTime) >= (int)Math.Round(Type2.GetValue() * 60))
            {
                ...
                {
                    player.immune = false;
                    player.Hurt(PlayerDeathReason.ByCustomReason($"{Player.nameLen} healed too little"), -amount, 0, false, false, -1, false, 999, 999f);
                }
            }
        }
