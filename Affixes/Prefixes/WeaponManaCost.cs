...
{
    public class WeaponManaCost : Prefix, ITieredStatFloatAffix
    {
        ...
        static float[] tiers = new float[] { 1.5f, 1.375f, 1.2f, 1f, 0.8f, 0.675f, 0.5f };
        ...
        {
            TooltipLine line = new TooltipLine(mod, "WeaponManaCost", $"[T{tierText}] {(multiplier < 1 ? '-' : '+')}{(int)Math.Round(Math.Abs((multiplier - 1) * 100))}% mana cost");
        ...
        {
            player.manaCost = player.manaCost * multiplier;
        }
