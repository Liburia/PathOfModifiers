...
{
    public class ArmorMagicDamage : Prefix, ITieredStatFloatAffix
    {
        ...
        {
            TooltipLine line = new TooltipLine(mod, "ArmorMagicDamage", $"[T{tierText}] {(multiplier < 1 ? '-' : '+')}{(int)Math.Round(Math.Abs((multiplier - 1) * 100))}% magic damage");
        ...
        {
            player.magicDamage += multiplier - 1;
        }
