...
{
    public class AccessoryMeleeDamage : Prefix, ITieredStatFloatAffix
    {
        ...
        {
            TooltipLine line = new TooltipLine(mod, "AccessoryMeleeDamage", $"[T{tierText}] {(multiplier < 1 ? '-' : '+')}{(int)Math.Round(Math.Abs((multiplier - 1) * 100))}% melee damage");
        ...
        {
            player.meleeDamage += multiplier - 1;
        }
