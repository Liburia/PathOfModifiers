...
namespace PathOfModifiers.Affixes.Prefixes
{
    public class WeaponDamagePerPlayerBuff : Prefix, ITieredStatFloatAffix
    {
        ...
        {
            TooltipLine line = new TooltipLine(mod, "WeaponDamagePerPlayerBuff", $"[T{tierText}] {(multiplier < 1 ? '-' : '+')}{(int)Math.Round(Math.Abs((multiplier - 1) * 100))}% damage per player buff/debuff");
