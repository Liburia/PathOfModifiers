...
{
    public class AccessoryCritDamage : Prefix, ITieredStatFloatAffix
    {
        ...
        static float[] tiers = new float[] { 0.85f, 0.92f, 1f, 1.08f, 1.15f };
        ...
        {
            TooltipLine line = new TooltipLine(mod, "AccessoryCritDamage", $"[T{tierText}] {(multiplier < 1 ? '-' : '+')}{(int)Math.Round(Math.Abs((multiplier - 1) * 100))}% crit damage");
