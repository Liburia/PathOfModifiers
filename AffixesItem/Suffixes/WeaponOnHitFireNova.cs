...
namespace PathOfModifiers.AffixesItem.Suffixes
{
    public class WeaponOnHitFireNova : Suffix, ITieredStatFloatAffix
    {
        ...
        {
            return $"[T{tierText}] {(int)Math.Round(multiplier * 100)}% chance to release a fire nova on hit";
        }

        public override void OnHitNPC(Item item, Player player, NPC target, int damage, float knockBack, bool crit)
        {
            if (item == player.HeldItem && Main.rand.NextFloat(0, 1) < multiplier)
                SpawnNova(player, damage);
        }
        ...
        public override void ProjOnHitNPC(Item item, Player player, Projectile projectile, NPC target, int damage, float knockback, bool crit)
        {
            if (item == player.HeldItem &&
                Main.rand.NextFloat(0, 1) < multiplier &&
                (projectile.modProjectile == null ||
                !(projectile.modProjectile is INonTriggerringProjectile)))
                SpawnNova(player, damage);
        }
